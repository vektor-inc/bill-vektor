<?php
/**
 * Class BillItemCustomFieldsNonceTickTest
 *
 * Bill_Item_Custom_Fields::save_custom_fields() の nonce 検証テスト（issue #312）
 *
 ...
 * 利用者には「更新を押したのに請求品目だけ元に戻る」という形でしか見えない）。
 *
 ...
 * @package BillVektor
 */
class BillItemCustomFieldsNonceTickTest extends WP_UnitTestCase {
	...
	public function set_up() {
		...
		// fields_form() / save_custom_fields() が参照する投稿を用意する
	...
	public function tear_down() {
		...
		delete_post_meta( $this->post_id, 'bill_items' );
		wp_delete_post( $this->post_id, true );
		unset( $GLOBALS['post'] );

		remove_filter( 'nonce_life', array( $this, 'filter_nonce_life' ) );
		wp_set_current_user( 0 );
		unset( $_POST['bill_items'], $_POST['noncename__bill_fields'] );
	...
	/**
	 * Bill_Item_Custom_Fields::save_custom_fields() のテスト
	 *
	 * 編集画面を開いた時点（fields_form() 実行時）を基準の tick とし、
	 * 保存した時点（save_custom_fields() 実行時）の tick をずらして、
	 * 保存が成功・失敗する境界を検証する。
	 *
	 * @return void
	 */
	public function test_save_custom_fields() {
		...
		foreach ( $test_cases as $case ) {
			...
			update_post_meta( $this->post_id, 'bill_items', 'before_value' );

			// 編集画面を開いた時点の tick を固定して nonce フィールドを出力する
			$this->set_nonce_tick( $base_tick );
			ob_start();
			Bill_Item_Custom_Fields::fields_form();
			$html  = ob_get_clean();
			$nonce = $this->extract_nonce_value( $html, 'noncename__bill_fields' );
			...
			// フォーム送信を再現する
			$_POST['noncename__bill_fields'] = $nonce;
			$_POST['bill_items']             = 'after_value';

			// テスト関数実行
			Bill_Item_Custom_Fields::save_custom_fields( $this->post_id );

			// 期待値テスト
			$actual   = get_post_meta( $this->post_id, 'bill_items', true );
