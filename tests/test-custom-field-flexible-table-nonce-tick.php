<?php
/**
 * Class CustomFieldFlexibleTableNonceTickTest
 *
 * VK_Custom_Field_Builder_Flexible_Table::save_cf_value() の nonce 検証テスト（issue #312）
 *
 ...
 * （`wp_nonce_tick()` は `nonce_life` フィルターの戻り値から tick を算出する）。
 ...
 * @package BillVektor
 */
class CustomFieldFlexibleTableNonceTickTest extends WP_UnitTestCase {
	...
	public function set_up() {
		...
		// form_table_flexible() / save_cf_value() が参照する投稿を用意する
		$this->post_id   = self::factory()->post->create();
		$GLOBALS['post'] = get_post( $this->post_id );

		$this->custom_fields_array = array(
			'field_name'  => 'nonce_tick_flex_field',
			'items'       => array(
				'col1' => array( 'label' => '項目1' ),
			),
			'row_default' => 1,
		);
	}
	...
	public function tear_down() {
		...
		delete_post_meta( $this->post_id, 'nonce_tick_flex_field' );
		wp_delete_post( $this->post_id, true );
		unset( $GLOBALS['post'] );

		remove_filter( 'nonce_life', array( $this, 'filter_nonce_life' ) );
		wp_set_current_user( 0 );
		unset( $_POST['nonce_tick_flex_field'], $_POST['noncename__nonce_tick_flex_field'] );
	...
	/**
	 * VK_Custom_Field_Builder_Flexible_Table::save_cf_value() のテスト
	 *
	 * 編集画面を開いた時点（form_table_flexible() 実行時）を基準の tick とし、
	...
	public function test_save_cf_value() {
		...
		$nonce_field_name = 'noncename__' . $this->custom_fields_array['field_name'];

		foreach ( $test_cases as $case ) {
			// 前回保存値を既知の値にリセットしておく（保存されたかどうかを判定するため）
			update_post_meta( $this->post_id, 'nonce_tick_flex_field', 'before_value' );

			// 編集画面を開いた時点の tick を固定して nonce フィールドを出力する
			$this->set_nonce_tick( $base_tick );
			ob_start();
			VK_Custom_Field_Builder_Flexible_Table::form_table_flexible( $this->custom_fields_array );
			$html  = ob_get_clean();
			$nonce = $this->extract_nonce_value( $html, $nonce_field_name );
			...
			// フォーム送信を再現する
			$_POST[ $nonce_field_name ]     = $nonce;
			$_POST['nonce_tick_flex_field'] = 'after_value';

			// テスト関数実行
			VK_Custom_Field_Builder_Flexible_Table::save_cf_value( $this->custom_fields_array );

			// 期待値テスト
			$actual   = get_post_meta( $this->post_id, 'nonce_tick_flex_field', true );
