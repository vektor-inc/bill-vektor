...
// まとめた抽象テストケース（tests/test-post-type-filter.php と共有）。
 ...
 */
class SearchKeywordTest extends Bill_Document_List_TestCase {

	/**
	 * テスト前のセットアップ
	 *
	 * 共通の前提（既存投稿の削除・管理者ログイン・認証リダイレクトの解除）は
	 * 親クラス（Bill_Document_List_TestCase）に任せ、このクラス固有のテスト用書類のみ作成する。
	 *
	 * @return void
	 */
	...
	// テスト後のクリーンアップ（$_GET のリセット・作成した書類とユーザーの削除）は
	// 親クラス（Bill_Document_List_TestCase::tear_down()）が行うため、ここでは定義しない。
