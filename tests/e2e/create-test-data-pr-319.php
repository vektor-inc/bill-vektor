...
if ( ! in_array( $bill_e2e_319_env, array( 'local', 'development' ), true ) ) {
	WP_CLI::error( 'このスクリプトはローカル／開発環境専用です（現在: ' . $bill_e2e_319_env . '）。wp-config.php に define( \'WP_ENVIRONMENT_TYPE\', \'local\' ); を追加してください。' );
}
...
function bill_e2e_319_upsert_document() {
	...
	if ( $post_id ) {
		...
		// 発行日を現在時刻へ更新する。
		// 再利用のたびに最初の作成日時のまま据え置くと、実行のたびに一覧の並びが
		// 変わってしまい読みにくいため最新化している。
		// 注意: これは「一覧の1ページ目に残ること」の担保にはならない。実行順という
		// 外部条件に依存した担保は #321 が無くそうとしている「決め打ち」と同じ性質のため、
		// 一覧確認側（pr-319-view-auth.spec.js）は bill_keyword による件数非依存の
		// 絞り込みで対象を特定しており、この発行日更新の有無に依存していない。
